# 研究议程 — 苏格拉底对话整理
*Session 1: 2026年4月24日 — 评估框架、论文结构*
*Session 2: 2026年4月24日 — 数据设计、社区建设、MVP定义*
*详细对话记录见 `socratic-dialogue-session2.md`*
...
*Session 2 更新：MVP 定义已精确化*

**最小版本**：1篇文章，全流程跑通，能出证书 = MVP 完成
...
**证书路线**：PDF + 唯一码（初期）→ Open Badge（中期）→ 区块链可探索（不优先）

**研究规模**（第一轮数据收集）：5篇文章 × 20名学习者 × 完整评估流程
...
| 原则 | 内容 |
|------|------|
| 运营数据即研究数据 | 平台从第一天设计成研究数据库，匿名数据集对外开放 |
| xAPI 全用户收集 | BYOK 用户也收 xAPI（证书所需），不存在采样偏差 |
| 知情同意前置 | 注册时声明数据用途，正面表述"贡献学习科学" |
| 社区先于质量管控 | 初期容忍"为积分而来"的用户，防弹机制是第二阶段 |
| IEEE 品牌是最强信任机制 | 代码鲁棒 + IEEE 署名，不依赖区块链背书 |

## 可能的第五篇论文

**Data Paper**：开放匿名 xAPI 数据集
- 目标期刊：Journal of Learning Analytics（Data Paper 类型）
- 内容：数据集描述、收集方法、伦理声明、使用指南
- 价值：吸引外部研究者，放大整个议程的影响力
