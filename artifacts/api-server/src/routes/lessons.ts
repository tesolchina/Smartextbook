...
router.post("/lessons", async (req, res): Promise<void> => {
  ...
  await processLessonWithAI(lesson.id, chapterText, llmConfig);

  const [updated] = await db.select().from(lessonsTable).where(eq(lessonsTable.id, lesson.id));
  res.status(201).json(serializeLesson(updated));
...
router.post("/lessons/:id/chat", async (req, res): Promise<void> => {
  ...
  const { message, history, llmConfig } = body.data;
