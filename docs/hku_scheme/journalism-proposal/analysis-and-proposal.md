...
This project develops a Trio partnership to co-design a proactive AI scaffolding system grounded in a real student's exemplary analysis (the "Bess framework"), then pilots it in [Course code/title] at [Institution].

**Expected outcomes:**
- Measurable improvement in students' analytical depth scores on data journalism articles (pre/post rubric)
- Growth in ability to classify data sources hierarchically and evaluate methodological choices
- A validated, open-source AI scaffolding prompt library for data journalism education — replicable by any journalism programme
